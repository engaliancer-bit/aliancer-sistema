/*
  # CORREÇÃO CRÍTICA: Status Correto para production_orders
  
  ## Problema RAIZ Identificado!
  O trigger estava usando status='pending', mas a tabela production_orders
  aceita apenas: 'open', 'in_progress', 'completed', 'cancelled'
  
  ## Causa do Problema
  O INSERT no trigger estava falhando silenciosamente devido ao constraint:
  CHECK ((status = ANY (ARRAY['open'::text, 'in_progress'::text, 'completed'::text, 'cancelled'::text])))
  
  ## Solução
  - Trocar status='pending' para status='open'
  - 'open' é o status inicial correto para uma ordem recém-criada
  
  ## Impacto
  ✅ Trigger agora funcionará corretamente
  ✅ Ordens serão criadas ao aprovar orçamentos
  ✅ Composições serão processadas
  ✅ Sistema verificará estoque e criará ordens necessárias
  
  ## Teste
  1. Aprovar orçamento com composição
  2. Sistema criará ordens com status='open'
  3. Ordens aparecerão em production_orders
*/
      ...
      IF v_quote_item.item_type = 'product' AND v_quote_item.product_id IS NOT NULL THEN
        ...
        IF v_quantity_to_produce > 0 THEN
          ...
          IF NOT EXISTS (
            ...
            -- Criar ordem de produção com status CORRETO: 'open'
...
CORRIGIDO: Status correto "open" ao invés de "pending".';
