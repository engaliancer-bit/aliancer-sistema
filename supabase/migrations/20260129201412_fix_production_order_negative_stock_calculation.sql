/*
  ...
  ## Problema
  Quando o estoque de um produto está negativo (já reservado mas não produzido),
  o sistema calcula quantidade incorreta de produção.
  
  Exemplo:
  - Orçamento pede 6 unidades de uma composição com 1 Tirante
  - Estoque do Tirante: -8 unidades (já reservado)
  - Cálculo ERRADO: 6 - (-8) = 14 unidades
  - Cálculo CORRETO: 6 - 0 = 6 unidades
  
  ## Solução
  Considerar estoque negativo como zero no cálculo de quantidade a produzir.
  Usar GREATEST(estoque, 0) ao invés de apenas estoque.
  
  ## Impacto
  Ordens de produção agora terão quantidade correta mesmo quando estoque
  estiver negativo devido a reservas de entregas futuras.
*/

-- =====================================================
-- CORREÇÃO DA FUNÇÃO DE CRIAÇÃO AUTOMÁTICA
-- =====================================================
  ...
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    LOOP
      ...
      IF v_quote_item.item_type = 'product' AND v_quote_item.product_id IS NOT NULL THEN
        ...
        -- CORREÇÃO: Considerar estoque negativo como zero
        v_quantity_to_produce := v_quote_item.quantity - GREATEST(COALESCE(v_inventory_stock, 0), 0);
        
        PERFORM log_production_order_event(
          NEW.id,
          'INFO',
          format('Produto %s: Necessário=%s, Estoque=%s, Estoque efetivo=%s, A produzir=%s',
            v_product_record.name, v_quote_item.quantity, v_inventory_stock, 
            GREATEST(COALESCE(v_inventory_stock, 0), 0), v_quantity_to_produce),
          jsonb_build_object(
            'product_id', v_quote_item.product_id,
            'needed', v_quote_item.quantity,
            'in_stock_raw', v_inventory_stock,
            'in_stock_effective', GREATEST(COALESCE(v_inventory_stock, 0), 0),
      ...
      ELSIF v_quote_item.item_type = 'composition' AND v_quote_item.composition_id IS NOT NULL THEN
        ...
        LOOP
          ...
          PERFORM log_production_order_event(
            NEW.id,
            'INFO',
            format('Produto %s (da composição %s): Necessário=%s, Estoque=%s, Estoque efetivo=%s, A produzir=%s',
              v_composition_item.product_name, v_composition_name, v_quantity_required, 
              v_inventory_stock, GREATEST(COALESCE(v_inventory_stock, 0), 0), v_quantity_to_produce),
            jsonb_build_object(
...
IMPORTANTE: Considera estoque negativo como zero para evitar cálculos incorretos.';
