/*
  # Corrigir Compatibilidade com Estrutura Real da Tabela production_orders
  
  ## Problema Identificado
  O trigger estava usando campos que NÃO EXISTEM na tabela `production_orders`:
  - Tentava inserir em `quantity` → Campo correto: `total_quantity`
  - Tentava inserir em `priority` → Campo NÃO EXISTE
  - Tentava inserir `order_number` como TEXT → Campo é INTEGER
  ...
  - status, notes, deadline, etc.
  
  ## Solução
  - Usar `total_quantity` ao invés de `quantity`
  - Remover inserção de `priority` (campo não existe)
  - Gerar `order_number` como INTEGER
  - Calcular `remaining_quantity` = total_quantity - produced_quantity
  
  ## Teste
  1. Aprove um orçamento com composição
  2. Sistema deve criar ordens corretamente
  3. Verificar em production_orders
*/

-- Recriar função com estrutura correta
...
CORRIGIDO: Usando estrutura real da tabela (total_quantity, order_number INTEGER, etc).';
