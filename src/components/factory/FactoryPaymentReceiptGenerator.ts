...
export interface FactoryPaymentReceiptData {
  paymentId: string;
  paymentDate: string;
  value: number;
...
export async function generateFactoryPaymentReceipt(
  data: FactoryPaymentReceiptData,
  ...
  // --- DADOS DA VENDA / ORCAMENTO ---
  y += 10;
  const infoBoxHeight = data.installmentDescription ? 36 : 28;
  doc.setFillColor(245, 247, 252);
  doc.roundedRect(margin, y - 5, contentWidth, infoBoxHeight, 2, 2, 'F');
  doc.setDrawColor(...ALIANCER_BLUE_MID);
  doc.setLineWidth(0.3);
  doc.rect(margin, y - 5, contentWidth, infoBoxHeight);

  doc.setFillColor(...ALIANCER_BLUE);
  doc.rect(margin, y - 5, 3, infoBoxHeight, 'F');

  doc.setFontSize(8);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(80, 80, 80);
  doc.text('DADOS DA VENDA / ORCAMENTO', margin + 7, y + 1);
  ...
  doc.setFont('helvetica', 'normal');
  const customerText = data.customerName.length > 38 ? data.customerName.substring(0, 38) + '...' : data.customerName;
  doc.text(customerText, col1X + 17, y + 9);

  doc.setFont('helvetica', 'bold');
  doc.text('Venda Nº:', col2X, y + 9);
  doc.setFont('helvetica', 'normal');
  doc.text(data.saleNumber || '-', col2X + 22, y + 9);

  doc.setFont('helvetica', 'bold');
  doc.text('Origem:', col1X, y + 17);
  doc.setFont('helvetica', 'normal');
  doc.text(`${data.originType} - ${data.originReference}`, col1X + 17, y + 17);

  if (data.installmentDescription) {
    doc.setFont('helvetica', 'bold');
    doc.text('Parcela:', col2X, y + 17);
    doc.setFont('helvetica', 'normal');
    const parcelaText = `${data.installmentNumber ? `${data.installmentNumber} - ` : ''}${data.installmentDescription}`;
    doc.text(parcelaText, col2X + 17, y + 17);
  }
  ...
  doc.setFontSize(8);
  doc.setTextColor(80, 80, 80);
  doc.text('RESUMO FINANCEIRO DO ORCAMENTO', margin + 7, y + 2);
  ...
  const filename = `recibo_fabrica_${data.saleNumber || receiptRef}_${data.paymentDate.split('T')[0].replace(/-/g, '')}.pdf`;
  doc.save(filename);
}
